...
export function MilestoneForm({ milestones, onAdd, onRemove }: MilestoneFormProps) {
  ...
  function handleSubmit(e: FormEvent) {
    ...
    onAdd({
      ...
      label: label.trim() || (type === "accident" ? "Acidente" : "Marco"),
      date: new Date(date),
      type,
    });
    setLabel("");
  ...
  return (
    <div className="flex flex-col gap-3 rounded-lg border border-zinc-200 p-4 dark:border-zinc-800">
      <div>
        <h3 className="text-sm font-medium">Marcos manuais</h3>
        <p className="text-xs text-zinc-500">
          O Excel só tem encontros médicos. Adiciona aqui a data do acidente
          ou outros marcos que não vêm dos dados.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-wrap items-end gap-2">
        ...
        <label className="flex flex-col text-xs text-zinc-500">
          ...
          <input
            ...
            placeholder={hasAccident ? "Ex: Fim do tratamento" : "Data do acidente"}
