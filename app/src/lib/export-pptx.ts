...
export async function exportCaseToPptx(medicalCase: Case): Promise<void> {
  ...
  const rangeText =
    sorted.length > 0
      ...
      : "No dated encounters";
  title.addText(rangeText, { x: 0.6, y: 3.7, w: 12.1, h: 0.5, fontSize: 16, color: MUTED });
  title.addText("Medical Timeline", { x: 0.6, y: 6.7, w: 6, h: 0.4, fontSize: 11, color: RUST });
  ...
  if (milestones.length > 0) {
    ...
    slide.addText("Key Dates", { x: 0.6, y: 0.4, fontSize: 24, bold: true, color: INK });
    slide.addText(
      sortedMilestones.map((m, i) => ({
        text: `${m.label} — ${dateFormatter.format(m.date)}${m.notes ? `: ${m.notes}` : ""}`,
        options: { bullet: true, breakLine: i < sortedMilestones.length - 1 },
      })),
      { x: 0.6, y: 1.3, w: 12.1, h: 5.5, fontSize: 15, color: "3C362B", valign: "top" }
    );
  }
  ...
  for (let i = 0; i < sorted.length; i += ROWS_PER_SLIDE) {
    ...
    const headerRow = ["Date", "Provider / Facility", "Type", "Summary"].map((text) => ({
      text,
      options: { bold: true, fill: { color: HEADER_FILL }, color: INK },
    }));

    const dataRows = chunk.map((event) => [
      { text: dateFormatter.format(event.date) },
      {
        text: `${event.providers.join(", ") || "Not specified"}\n${
          event.facility || "Facility not specified"
        }`,
      },
      { text: event.recordType || event.medicineType || "" },
      { text: truncate(event.summary, SUMMARY_MAX_CHARS) },
    ]);
